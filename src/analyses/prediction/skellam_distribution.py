...
def run(country: str, league: str, season: str):
    ...
    if st.button("Run Model for All Matches", key="skellam_calc_button_all"):
        ...
        st.markdown(f"### {season} {league} - Week {last_week} Predictions (Skellam Distribution)")
        ...
        for _, match_row in last_week_matches.iterrows():
            ...
            if diffs is None or diff_probs is None or not np.any(diff_probs):
                st.warning(f"No score difference distribution to plot for {home_team} - {away_team}.")
                continue
