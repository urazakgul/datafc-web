...
def _play_type_fbregular(situation: str) -> str | None:
...
def run(country: str, league: str, season: str):
    ...
    shots_df["play_type"] = shots_df["situation"].apply(_play_type_fbregular)
    ...
    xg_pm["Regular xG"] = (xg_pm["Regular"] / xg_pm["matches_played"]).round(3)
    ...
    ax.set_title(
        f"{season} {league}\nRegular vs Fast-Break xG per Match\n(up to Week {max_week})",
        fontsize=16,
        fontweight="bold",
        pad=30,
        loc="center"
