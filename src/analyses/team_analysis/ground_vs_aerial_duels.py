...
def run(country: str, league: str, season: str):
    ...
    parenthesis_keywords = ["Ground duels", "Aerial duels"]
    ...
    master_df = master_df[
        master_df["stat_name"].isin(["Ground duels", "Aerial duels"])
    ]
    ...
    tmp = (
        result_all_stats_df
        .assign(
            wins=result_all_stats_df["stat_value"].str.extract(r"^\s*(\d+)\s*/\s*(\d+)\s*$")[0].astype(int),
            attempts=result_all_stats_df["stat_value"].str.extract(r"^\s*(\d+)\s*/\s*(\d+)\s*$")[1].astype(int),
        )[["team_name", "stat_name", "wins", "attempts"]]
    )

    agg = (
        tmp.groupby(["team_name", "stat_name"], as_index=False)
           .sum(numeric_only=True)
    )

    agg["win_pct"] = np.where(
        agg["attempts"] > 0,
        agg["wins"] / agg["attempts"] * 100,
        np.nan
    )

    agg = agg.sort_values(["team_name", "stat_name"]).reset_index(drop=True)

    pivot_pct = (
        agg.pivot(index="team_name", columns="stat_name", values="win_pct")
    ...
    teams_df = (
        ...
        .rename(columns={
            "team_name": "team_name",
            "Ground duels": "Ground Duel Win (%)",
            "Aerial duels": "Aerial Duel Win (%)"
        })
        .dropna(subset=["Ground Duel Win (%)", "Aerial Duel Win (%)"])
    )
    ...
    x_col = "Ground Duel Win (%)"
    y_col = "Aerial Duel Win (%)"
    ...
    max_y, min_y = (float(np.max(y)), float(np.min(y))) if y.size else (100.0, 0.0)
    ...
    y_med = float(np.nanmedian(y)) if y.size else 50.0
