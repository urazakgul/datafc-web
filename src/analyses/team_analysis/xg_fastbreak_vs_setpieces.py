...
def _play_type_fb_setpieces(situation: str, exclude_penalties: bool) -> str | None:
...
def run(country: str, league: str, season: str):
    ...
    shots_df["play_type"] = shots_df["situation"].apply(
        lambda s: _play_type_fb_setpieces(s, exclude_penalties)
    )
    ...
    for col in ["Fast-Break", "Set Pieces"]:
    ...
    xg_pm["Fast-Break xG"] = (xg_pm["Fast-Break"] / xg_pm["matches_played"]).round(3)
    ...
    if x.size:
        max_x, min_x = float(np.max(x)), float(np.min(x))
    else:
        max_x, min_x = 1.0, 0.0
    if y.size:
    ...
    ax.set_title(
        f"{season} {league}\nFast-Break vs Set Pieces xG per Match{excl_txt}\n(up to Week {max_week})",
        fontsize=16,
        fontweight="bold",
        pad=30,
        loc="center"
